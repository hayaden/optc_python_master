...
def register_user(language='ko'):
    """Registers a new user with the server."""
    url = f"{config.get_api_url()}/users/register"
    user_data = build_user_data(language)
    ...
    if response.ok:
        ...
        print(f'✅ [{language.upper()}] 계정 생성 완료')
    else:
        print(response.status_code)
        print(f'❌ [{language.upper()}] 계정 생성 실패')
        sys.exit(0)

def login_user(language='ko'):
...
def build_user_data(language='ko'):
    ...
    user_data = (
        ...
        f'"locale":"{locale}"' if config.VERSION == 'gb' else ''
    )
